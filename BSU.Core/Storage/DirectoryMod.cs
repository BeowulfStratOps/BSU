...
namespace BSU.Core.Storage
{
    ...
    public class DirectoryMod : IStorageMod
    {
        ...
        {
            { typeof(VersionHash), async ct => await _jobManager.Run($"Version Hash {Dir.FullName}", () => VersionHash.CreateAsync(this, ct), ct)},
            { typeof(MatchHash), async ct => await _jobManager.Run($"Match Hash {Dir.FullName}", () => MatchHash.CreateAsync(this, ct), ct)},
        };
    }
}

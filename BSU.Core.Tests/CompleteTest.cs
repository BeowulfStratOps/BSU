...
namespace BSU.Core.Tests
{
    public class CompleteTest : LoggedTest
    {
        ...
        private void Load()
        {
            ...
            model.Load();
            worker.DoWork();
            var storageMod1 = model.Storages.Single(s => s.ToString() == "storage1").Mods
                .Single(m => m.ToString() == "mod5");
            var repoMod1 = model.Repositories.Single(s => s.ToString() == "repo1").Mods
                .Single(m => m.ToString() == "mod1");
